...
namespace GraniteHouse.Areas.Admin.Controllers
{
    ...
    public class AppointmentsController : Controller
    {
        ...
        public async Task<IActionResult> Index(int productPage = 1, string searchName=null, string searchEmail = null, string searchPhone = null, string searchDate = null)
        {
            ...
            var claimssIdentity = (ClaimsIdentity)this.User.Identity;
            var claim = claimssIdentity.FindFirst(ClaimTypes.NameIdentifier);
            ...
            appointmentVM.Appointments = _db.Appointments
                .Include(a => a.SalesPerson).ToList();

            if (User.IsInRole(SD.AdminEndUser))
            {
                appointmentVM.Appointments = appointmentVM.Appointments
                                                .Where(a => a.SalesPersonId == claim.Value).ToList();
            }
        ...
        public async Task<IActionResult> Edit(int id, AppointmentDetailsViewModel objAppointmentVM)
        {
            if (ModelState.IsValid)
            {
                ...
                var appointmentFromDb = _db.Appointments
                                        .Where(a => a.Id == objAppointmentVM.Appointment.Id)
                                        .FirstOrDefault();
